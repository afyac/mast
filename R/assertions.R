...
assert_single <-
  function(x,
           message = "%s must be a single value",
           name = deparse(substitute(x))) {
...
assert_neg <-
  function(x,
           zero_allowed = TRUE,
           message1 = "%s must be less than or equal to zero",
...
assert_limit <-
  function(x,
           message = "%s must be a valid limit, i.e. two increasing values",
           name = deparse(substitute(x))) {
