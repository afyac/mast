...
f_plot_missing_data <- function(data) {
  ...
  data <- data |>
    dplyr::mutate(dplyr::across(-Year, ~dplyr::if_else(is.na(.), 0, 1))) |>
    dplyr::group_by(Year) |>
    dplyr::summarise_all(.funs = mean, na.rm = TRUE) |>
    tidyr::pivot_longer(-Year, names_to="variable", values_to="value")

  # plot missing data
  plot <- ggplot2::ggplot(data = data, ggplot2::aes(x = Year, y =
                                                      forcats::fct_rev(variable))) +
    ggplot2::geom_tile(ggplot2::aes(fill=value), colour = "grey80", show.legend = TRUE) +
    ggplot2::ylab("") +
